...
class SørAmerikaQuestionsActivity : AppCompatActivity(), View.OnClickListener {
    ...
    val sørAmerika = listOf<String>("Argentina", "Bolivia", "Brasil", "Chile", "Colombia",
            "Ecuador", "Guyana", "Paraguay", "Peru", "Surinam", "Uruguay", "Venezuela")

    var sørAmerika_draw = mutableListOf<String>("Argentina", "Bolivia", "Brazil", "Chile", "Colombia",
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        for (i in (0 until (sørAmerika_draw.size))){
            sørAmerika_draw[i] = sørAmerika_draw[i].decapitalize()
            sørAmerika_draw[i] = "flag_of_" + sørAmerika_draw[i]
        }
    ...
    private fun submit(){
        if (mSelectedOptionPosition == 0){
            if (attempts == 0){
                MediaPlayer.create(this@SørAmerikaQuestionsActivity, R.raw.error).start()
            }else{
                mCurrentPosition += 1
                attempts = 0
            ...
            val wrong_music = listOf<MediaPlayer>(
                MediaPlayer.create(this@SørAmerikaQuestionsActivity, R.raw.thud),
                //MediaPlayer.create(this@SørAmerikaQuestionsActivity, R.raw.bruh),
                //MediaPlayer.create(this@SørAmerikaQuestionsActivity, R.raw.augh)
            )
            if (attempts < 1){
                if(question!!.correctAnswer != mSelectedOptionPosition){
                    ...
                    MediaPlayer.create(this@SørAmerikaQuestionsActivity, R.raw.correct).start()
                    mCorrectAnswers += 1
                }
    ...
    private fun getQuestions(): ArrayList<Question>{
        ...
        val flaggNavnList = sørAmerika
        val drawNameList = sørAmerika_draw
        var antall = 0
